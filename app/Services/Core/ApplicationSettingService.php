...
class ApplicationSettingService
{
    ...
    protected $settingsConfigurations;
    protected $type;
    protected $errorMessages = [];

    public function __construct(ApplicationSettingInterface $applicationSetting)
    {
        $this->applicationSetting = $applicationSetting;
        $this->settingsConfigurations = config("applicationsettings.settings");
    }
    ...
    public function update(Request $request, $type)
    {
        ...
        if (!empty($this->errorMessages)) {
            return [
                ...
                SERVICE_RESPONSE_MESSAGE => __('Invalid data in field(s).'),
                'errors' => $this->errorMessages,
                'inputs' => $settingsRequest
            ];
        }

        $existingSettingsFromDatabase = $this->applicationSetting->getBySlugs(array_keys($settingsRequest));
        $updateAbleSettings = array_diff_assoc($settingsRequest, $existingSettingsFromDatabase);
//        dd($existingSettingsFromDatabase, $settingsRequest);
        ...
        if (!empty($updateAbleSettings)) {
            ...
            foreach ($updateAbleSettings as $field => $value) {
                ...
                if ($this->applicationSetting->updateOrCreate($attributes, $conditions)) {
                    $updateCount++;
                }
            }

            if ($updateCount > 0) {
                $cachedSettings = Cache::get("application_settings");
                ...
                Cache::forever("application_settings", $cachedSettings);
    ...
    {
        $fieldConfiguration = $this->settingsConfigurations[$this->type][$field];
    ...
    protected function _validate($field, $value, $rule)
    {
        switch ($rule) {
            ...
            case strpos($rule, 'size:') !== false:
                ...
                $imageSize = $value->getClientSize() / 1024;
    ...
    private function uploadImages($uploadAbleImages, & $updateAbleSettings)
    {
        ...
        foreach ($uploadAbleImages as $field => $file) {
            ...
            $width = isset($this->settingsConfigurations[$this->type][$field]['width']) ? $this->settingsConfigurations[$this->type][$field]['width'] : null;
            $height = isset($this->settingsConfigurations[$this->type][$field]['height']) ? $this->settingsConfigurations[$this->type][$field]['height'] : null;
    ...
    public function loadForm($settingGroup = null, $viewOnly = false)
    {
        $output = '';
        $dataconfig = config('applicationsettings.settings');
        if (empty($settingGroup)) {
            $settingGroup = [];
            foreach ($dataconfig as $dc) {
                $settingGroup = array_merge($settingGroup, $dc);
            }
        } elseif (is_array($settingGroup)) {
            $settingGroup = array_intersect_key($dataconfig, array_flip($settingGroup));
        } else {
            $settingGroup = config('applicationsettings.settings.' . $settingGroup);
        }
        $common_wrapper = config('applicationsettings.common_wrapper');
        foreach ($settingGroup as $key => $value) {
            $common_input_options = config('applicationsettings.common_' . $value['field_type'] . '_input_wrapper');
...
//            dd(old('adminSettings.lang'));
        ...
        return ['html' => new HtmlString($output), 'settingSections' => array_keys($dataconfig)];
    }
    ...
    private function _image($key, $data_array, $input_class, $value_data, $place_holder, $input_start_tag, $input_end_tag)
    {
        ...
        $image = '<div class="fileinput fileinput-new" data-provides="fileinput">
                          <div class="fileinput fileinput-new" data-provides="fileinput">
                              <div class="fileinput-new img-thumbnail" style="width: 100px; height: 80px;">
                                <img src="' . get_image($value_data) . '"  alt="">
                              </div>
                          <div class="fileinput-preview fileinput-exists img-thumbnail" style="max-width: 100px; max-height: 80px;"></div>
                          <div>
