        ...
        foreach ($allRoutes as $routeName => $routeData) {
            ...
            if ($isMenuable &&
                (
                    ...
                    (in_array('verification.permission', $middleware) && Auth::user() && Auth::user()->is_email_verified == EMAIL_VERIFICATION_STATUS_INACTIVE && settings('require_email_verification') == ACTIVE_STATUS_ACTIVE
                    )
                )
            ) {
